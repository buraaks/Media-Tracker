<template>
  ...
  >
    <!-- Poster -->
    <div class="relative w-[110px] shrink-0 bg-white/5 overflow-hidden max-sm:w-[90px]">
      <img
        v-if="item.image"
        :src="item.image"
        :alt="item.title"
    ...
    <div class="flex-1 p-4 flex flex-col gap-2 min-w-0 max-sm:p-3">
      <h3 class="text-white/90 font-semibold text-[15px] leading-snug line-clamp-2 group-hover:text-white transition-colors">
        {{ item.title }}
      </h3>

      <div class="flex items-center gap-3 flex-wrap">
        <span v-if="item.year !== '-'" class="text-white/40 text-xs">{{ item.year }}</span>
      ...
      <p v-if="item.extra" class="text-white/35 text-xs leading-relaxed line-clamp-2 mt-auto">
        {{ item.extra }}
      </p>
    </div>
